...
function addLike(recipe){
  if (recipe.isLiked) {
    ...
    axios.post('https://6d2a810cc03d9116.mokky.dev/like', recipe)
      .then(response => {recipe.serverId = response.data.id;})
      .catch(error => {
        console.error(error);
      });
  }
  ...
  <div class="main__recipe__container">
    ...
    <div class="main__recipe__wrapp">
      <div class="recipe" v-for="(recipe, index) in props.recipe" :key="index">
        ...
        <img :class="{active : recipe.isLiked}" class="like" src="/src/assets/image/nolike.png" alt="" @click="addLike(recipe, index)"/>
      </div>
    </div>
